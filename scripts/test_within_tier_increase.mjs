...
console.log(
  `  Matrix Evaluation: resolveApprover("${prInput.category}", ${prInput.estimated_cost})`,
);
console.log(
  `  Derived Approver Role: "${prRouting.approverRole}" (escalated=${prRouting.escalated})`,
);
...
console.log(
  `  PO Input Amount: Price ₹${poInput.price} + Taxes ₹${poInput.taxes} = Total ₹${poTotalValue.toLocaleString("en-IN")}`,
);
...
console.log(
  `  Matrix Re-evaluation in createPo: resolveApprover("${prApproved.category}", ${poTotalValue})`,
);
console.log(
  `  Derived PO Approver Role: "${poRouting.approverRole}" (escalated=${poRouting.escalated})`,
);
