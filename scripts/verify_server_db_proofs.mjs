...
const poStateAfter = serverFnApprovePo(
  "usr-pc-member-01",
  ["purchase_committee"],
  poStateBefore.id,
  "Approved by Purchase Committee in compliance with SOP §8.5",
);
...
const prResubmittedState = serverFnResubmitPr("usr-faculty-01", prReturnedState.id, {
  justification:
    "Updated justification: Consumables needed for 120 incoming lab students in Semester 1",
});
...
console.log(
  "3. AFTER STATE (Direct DB SELECT on purchase_requisitions showing status reset to pending_approval):",
);
