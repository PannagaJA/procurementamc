...
// -------------------------------------------------------------
function validateGrnCreation({
  po,
  deliveryChallan,
  items,
  securityVerified,
  technicalVerified,
  requiresTechnicalAcceptance,
}) {
  if (!po) {
  ...
  if (!deliveryChallan) {
    throw new ProcurementRuleError(
      "NO_DELIVERY_CHALLAN",
      "Cannot create a Goods Receipt Note (GRN) without a recorded Delivery Challan (SOP §8.6).",
    );
  }
  if (!securityVerified) {
    throw new ProcurementRuleError(
      "SECURITY_VERIFICATION_REQUIRED",
      "Gate security inspection verification is mandatory prior to GRN generation.",
    );
  }
  if (requiresTechnicalAcceptance && !technicalVerified) {
    throw new ProcurementRuleError(
      "TECHNICAL_VERIFICATION_REQUIRED",
      "Technical acceptance by inspecting engineer is mandatory prior to GRN generation for this category.",
    );
  }
  ...
  if (cumulativeDelivered > po.ordered_quantity) {
    throw new ProcurementRuleError(
      "CUMULATIVE_OVERDELIVERY",
      `Cumulative delivered quantity (${cumulativeDelivered}) exceeds PO ordered quantity (${po.ordered_quantity}).`,
    );
  }

  const acceptedValue = items.reduce(
    (sum, item) =>
      sum + Number(item.qty_accepted || 0) * Number(item.unit_price || po.unit_price || 0),
    0,
  );
...
function recordPayment({ invoice, paymentAmount, paymentMode, externalRef, po }) {
  ...
  const updatedPoStatus =
    Number(paymentAmount) >= Number(po.total_value) ? "closed" : "partially_closed";
...
function requestEmergencyProcurement({
  pr,
  requestedBy,
  description,
  estimatedCost,
  ...
  const capCheck = checkAnnualCap({
    financialYear: fy,
    newAmount: estimatedCost,
    runningTotal: ledgerRunningTotal,
  });
  if (!capCheck.allowed) {
    throw new ProcurementRuleError(
...
function approveEmergencyProcurement({
  emergency,
  userRole,
  isPostFacto,
  registerEntryAt,
  ...
  if (userRole !== "evp" && userRole !== "Executive Vice President") {
    throw new ProcurementRuleError(
      ...
      "Only Executive Vice President (EVP) can authorize Emergency Procurements (SOP §9).",
    );
  }
...
function applyVendorRating({ vendor, scores, reviewerId, evpSignOffId }) {
  ...
  if (outcome === "suspended") {
    ...
    if (!evpSignOffId) {
      throw new ProcurementRuleError(
        "EVP_APPROVAL_REQUIRED_FOR_DEBARMENT",
        "Debarment/Blacklisting of a vendor strictly requires EVP approval.",
      );
    }
...
function validateVendorSelectionForRfq(vendor) {
  if (vendor.status === "blacklisted" || vendor.status === "debarred") {
    throw new ProcurementRuleError(
      "DEBARRED_VENDOR_BLOCKED",
      `Vendor "${vendor.name}" is debarred/blacklisted and cannot participate in RFQs.`,
    );
  }
  if (vendor.status === "suspended") {
    throw new ProcurementRuleError(
      "SUSPENDED_VENDOR_BLOCKED",
      `Vendor "${vendor.name}" is currently suspended and cannot be invited to RFQs.`,
    );
  }
  return true;
}
...
try {
  ...
  assert.throws(
    ...
    (err) =>
      err.code === "CUMULATIVE_OVERDELIVERY" || err.message.includes("exceeds PO ordered quantity"),
  );
...
try {
  ...
  pass(
    `Mismatched invoice (billed ₹50k vs GRN accepted ₹40k) is held with reason: "${matchResult.hold_reason}".`,
  );
  ...
  assert.throws(
    ...
    (err) =>
      err.code === "MATCH_REQUIRED_FOR_APPROVAL" || err.message.includes("Cannot approve invoice"),
  );
...
try {
  ...
  // Without certificate
  const matchNoCert = runThreeWayMatch({
  ...
  // With certificate
  const matchWithCert = runThreeWayMatch({
    invoice: serviceInvoice,
    po: servicePo,
    grn: null,
    isServicePo: true,
    completionCertUrl: "https://docs.amc.org/cert123.pdf",
  });
...
try {
  ...
  assert.throws(
    () =>
      recordPayment({
        invoice: unapprovedInvoice,
        paymentAmount: 50000,
        paymentMode: "neft",
        externalRef: "UTR123",
        po: { total_value: 50000 },
      }),
    (err) =>
      err.code === "UNAPPROVED_INVOICE_PAYMENT_BLOCKED" ||
      err.message.includes("Cannot record payment"),
  );
...
try {
  ...
  assert.throws(
    ...
    (err) =>
      err.code === "UNAUTHORIZED_EVP_REQUIRED" ||
      err.message.includes("Only Executive Vice President"),
  );
  ...
  assert.throws(
    ...
    (err) =>
      err.code === "POST_FACTO_EXPIRED" ||
      err.message.includes("Post-facto ratification timed out"),
  );
  pass(
...
try {
  // Band 1: Preferred (>= 85)
  const score1 = computeWeightedScore({
    sectionA: 95,
    sectionB: 90,
    sectionC: 90,
    sectionD: 85,
  ...
  const score2 = computeWeightedScore({
    sectionA: 75,
    sectionB: 75,
    sectionC: 70,
    sectionD: 75,
  ...
  const score3 = computeWeightedScore({
    sectionA: 60,
    sectionB: 60,
    sectionC: 60,
    sectionD: 60,
  ...
  assert.strictEqual(score3, 60.0);
  ...
  const score4 = computeWeightedScore({
    sectionA: 45,
    sectionB: 50,
    sectionC: 45,
    sectionD: 50,
  ...
  assert.strictEqual(score5, 29.0);
  ...
  const scoreNoE = computeWeightedScore({
    sectionA: 90,
    sectionB: 80,
    sectionC: 80,
    sectionD: 70,
  ...
  assert.throws(
    ...
    (err) =>
      err.code === "EVP_APPROVAL_REQUIRED_FOR_DEBARMENT" ||
      err.message.includes("requires EVP approval"),
  );
