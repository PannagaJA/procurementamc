...
async function assertPrApproved(db, prId) {
  ...
  if (data.status !== "approved") {
    throw ruleError(
      "pr_not_approved",
      `Purchase requisition is "${data.status}". No procurement step may proceed until approved.`,
    );
  }
}
...
async function assertNoPoSplitting(db, prId, newValue, vendorId, departmentId) {
  ...
  if (approved > 0 && projected > approved) {
    throw ruleError(
      "po_splitting",
      `Purchase orders would total ₹${projected}, above approved requisition value of ₹${approved}.`,
    );
  }

  if (vendorId) {
    const { data: recentVendorPos } = await db
      .from("purchase_orders")
      .select()
      .eq("vendor_id", vendorId)
      .gte("created_at", "30days")
      .neq("status", "cancelled");
...
function validateCsRecommendation(recommendedVendorId, lowestVendorId, rationale) {
  ...
  if (!isLowest && (!rationale || !rationale.trim())) {
    throw ruleError(
      "non_lowest_rationale_required",
      "Non-lowest price rationale is mandatory when recommending a vendor other than lowest bidder (§7.2).",
    );
  }
  return true;
}
...
try {
  ...
  if (e.code === "non_lowest_rationale_required") {
    pass(
      `Submitting CS recommending non-lowest vendor with empty rationale is rejected: [${e.code}]`,
    );
  } else {
...
const validCs = validateCsRecommendation(
  "vendor-premium",
  "vendor-cheap",
  "Superior 3-year warranty and OEM calibration standards.",
);
...
function validatePoCreation(type, csStatus, rcActive) {
  if (type === "regular" && csStatus !== "approved") {
    throw ruleError(
      "cs_not_approved",
      `Cannot create PO: Comparative Statement is "${csStatus}", not approved.`,
    );
  }
...
pass(
  `Amending PO from ₹8,000 (Principal limit ₹10k) to ₹15,000 escalates to Purchase Committee & forces requires_reapproval=true`,
);
...
try {
  ...
  if (e.code === "po_splitting_window") {
    pass(
      `Splitting ₹18,000 requirement into two ₹9,000 POs to same vendor within 30 days is blocked: [${e.code}]`,
    );
  } else {
    fail(`Wrong error: ${e.message}`);
  }
