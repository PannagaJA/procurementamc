...
function runThreeWayMatch({ invoice, po, grn, isServicePo, completionCertUrl, tolerance = 0.01 }) {
  if (isServicePo) {
    if (!completionCertUrl) {
      return {
        ...
        hold_reason:
          "Service PO match failed: Mandatory Work Completion / Milestone Certificate is missing in lieu of GRN (SOP §I2).",
      };
    }
  ...
  if (!grn) {
    return {
      match_status: "on_hold",
      hold_reason:
        "Three-way match failed: No approved Goods Receipt Note (GRN) found for this Purchase Order.",
    };
  }
...
function assertEmergencyCapNotExceeded({
  runningTotal,
  newAmount,
  capLimit = 1000000,
...
function assertNoPoSplitting({
  ...
  const cumulativeOnPr =
    existingPosOnPr.reduce((sum, po) => sum + Number(po.total_value || 0), 0) + Number(newPoAmount);
  ...
  const rolling30dTotal =
    recentVendorPos.reduce((sum, po) => sum + Number(po.total_value || 0), 0) + Number(newPoAmount);
...
const matrixBoundaries = [
  { category: "small_value", txn: 2000, mtd: 0, label: "Small Value ₹2,000 (exact txn boundary)" },
  {
    ...
    txn: 2001,
    mtd: 0,
    label: "Small Value ₹2,001 (1 rupee over HOD txn limit)",
  },
  {
    category: "small_value",
    txn: 5000,
    mtd: 0,
    label: "Small Value ₹5,000 (exact Principal txn boundary)",
  },
  {
    category: "small_value",
    txn: 5001,
    mtd: 0,
    label: "Small Value ₹5,001 (1 rupee over Principal txn limit)",
  },
  {
    category: "small_value",
    txn: 2000,
    mtd: 28000,
    label: "Small Value ₹2,000 with MTD ₹28,000 (Total ₹30,000 exact month cap)",
  },
  {
    category: "small_value",
    txn: 2001,
    mtd: 28000,
    label: "Small Value ₹2,001 with MTD ₹28,000 (Total ₹30,001 - 1 rupee over month cap)",
  },
  {
    category: "equipment_asset",
    txn: 10000,
    mtd: 0,
    label: "Equipment ₹10,000 (exact PC txn boundary)",
  },
  {
    category: "equipment_asset",
    txn: 10001,
    mtd: 0,
    label: "Equipment ₹10,001 (1 rupee over PC txn limit)",
  },
  {
    category: "equipment_asset",
    txn: 5000,
    mtd: 45000,
    label: "Equipment ₹5,000 with MTD ₹45,000 (Total ₹50,000 exact month cap)",
  },
  {
    category: "equipment_asset",
    txn: 5001,
    mtd: 45000,
    label: "Equipment ₹5,001 with MTD ₹45,000 (Total ₹50,001 - 1 rupee over month cap)",
  },
  {
    category: "routine_consumable",
    txn: 20000,
    mtd: 80000,
    label: "Routine Consumable with MTD ₹80,000 (Total ₹1,00,000 exact month cap)",
  },
  {
    category: "routine_consumable",
    txn: 20001,
    mtd: 80000,
    label: "Routine Consumable with MTD ₹80,000 (Total ₹1,00,001 - 1 rupee over month cap)",
  },
];
...
for (const b of matrixBoundaries) {
  ...
  console.log(
    `  Output: approverRole="${res.approverRole}", escalated=${res.escalated}, minQuotes=${res.minQuotations}, escalationReason=${JSON.stringify(res.escalationReason)}`,
  );
  console.log("");
}
...
function testApprovePoAsViewer() {
  ...
  if (!callerRoles.includes(requiredRole) && !callerRoles.includes("admin")) {
    throw new ProcurementRuleError(
      "FORBIDDEN_ROLE",
      `You are not authorized to approve this Purchase Order. Required role: "${requiredRole}". Your active roles: ["${callerRoles.join('", "')}"]`,
    );
  }
}
try {
...
function testRecordPaymentAsStores() {
  ...
  if (!callerRoles.includes("finance") && !callerRoles.includes("admin")) {
    throw new ProcurementRuleError(
      "FORBIDDEN_FINANCE_ROLE",
      `Payment disbursement can only be recorded by Finance officers. Your active roles: ["${callerRoles.join('", "')}"]`,
    );
  }
}
try {
...
function testApproveEmpanelmentNonEvp() {
  ...
  if (!callerRoles.includes("evp") && !callerRoles.includes("admin")) {
    throw new ProcurementRuleError(
      "FORBIDDEN_EVP_REQUIRED",
      `You are not permitted to approve vendor empanelment. Required role: evp. Your active roles: ["${callerRoles.join('", "')}"]`,
    );
  }
}
try {
...
function testApproveEmergencyNonEvp() {
  ...
  if (!callerRoles.includes("evp") && !callerRoles.includes("admin")) {
    throw new ProcurementRuleError(
      "UNAUTHORIZED_EVP_REQUIRED",
      `Only Executive Vice President (EVP) can authorize Emergency Procurements (SOP §9). Your active roles: ["${callerRoles.join('", "')}"]`,
    );
  }
}
try {
...
console.log(
  '[ATTEMPT 3.5] Attempting Payment Recording directly on unapproved invoice (status: "on_hold")',
);
...
const mismatchGrn = {
  id: "grn-101",
  grn_number: "GRN-2026-0101",
  status: "accepted",
  accepted_value: 40000,
};
...
console.log(
  `  GRN Physical Accepted Value:    ₹${mismatchGrn.accepted_value.toLocaleString("en-IN")}`,
);
console.log(
  `  Vendor Claimed Invoice Amount:  ₹${mismatchInvoice.invoice_amount.toLocaleString("en-IN")}`,
);
...
console.log(
  `[STATE] Current FY 2026-27 Emergency Ledger Running Total: ₹${currentLedgerTotal.toLocaleString("en-IN")}`,
);
...
const capCheckAllowed = checkAnnualCap({
  financialYear: "2026-27",
  newAmount: 100000,
  runningTotal: currentLedgerTotal,
});
console.log(`  Raw Response: ${JSON.stringify(capCheckAllowed, null, 2)}`);
console.log(
  `  Outcome: ALLOWED (allowed=${capCheckAllowed.allowed}, remaining_after=₹${capCheckAllowed.remaining_after})`,
);
console.log("");

// Case B: New request of ₹1,00,001 (Total = ₹10,00,001 - ONE RUPEE OVER CAP)
console.log(
  "[TEST 5.2] Submitting Request of ₹1,00,001 (Projected Total: ₹10,00,001 - 1 RUPEE OVER CAP)",
);
try {
...
console.log(
  `  Prior PO (10 days ago): ₹${priorPo.total_value.toLocaleString("en-IN")} (Approved under delegation)`,
);
console.log(
  `  New PO Attempt:         ₹${newPoAmount.toLocaleString("en-IN")} (Attempted under delegation)`,
);
console.log(
  `  Combined 30-Day Total:  ₹${(priorPo.total_value + newPoAmount).toLocaleString("en-IN")} (Crosses ₹10,000 delegation limit)`,
);
console.log("");
