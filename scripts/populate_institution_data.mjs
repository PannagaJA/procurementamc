...
async function runPopulation() {
    ...
    .insert([
      {
        ...
        justification:
          "Procurement of 10x High-Performance GPU Workstations for M.Tech & PhD AI Research Compute cluster.",
  ...
  if (pr1) {
    ...
    await supabase.from("pr_approvals").insert([
      {
        pr_id: pr1.id,
        approver_id: hodCseId,
        approver_role: "hod",
        status: "approved",
        comments: "Recommended. Needed for AI Lab expansion.",
      },
      {
        pr_id: pr1.id,
        ...
        status: "approved",
        comments: "Concurred and forwarded for Capital Budget release.",
      },
      {
        pr_id: pr1.id,
        approver_id: evpId,
        approver_role: "evp",
        status: "approved",
        comments: "Approved within Annual Capital Expenditure Allocation.",
      },
    ]);
  }
    ...
    .insert([
      {
        ...
        justification:
          "Replacement of defective optical projectors with 75-inch Interactive 4K Touch Panels in Seminar Hall.",
    ...
    await supabase.from("pr_line_items").insert([
      {
        pr_id: pr2.id,
        description:
          "75-inch 4K UHD Interactive Flat Panel Display with Built-in Android & Windows OPS",
        unit: "units",
        qty_required: 2,
        qty_in_stock: 0,
      ...
      .insert([
        {
          ...
          status: "approved",
          comments:
            "Verified Seminar Hall requirements. Forwarded to Principal for financial sanction.",
        },
      ]);
  }

  // PR 3: Campus MATLAB & Simulink Licenses (₹8,00,000, Pending Purchase Committee in "My Approvals")
  const { data: pr3 } = await supabase
    .from("purchase_requisitions")
    .insert([
      {
        ...
        justification:
          "Annual renewal of Campus-Wide 500-user MATLAB, Simulink, and 45 companion toolboxes.",
    ...
    .insert([
      {
        ...
        justification:
          "High-speed carbide cutting tools and drill bits for 4th Semester Manufacturing Lab sessions.",
    ...
    .insert([
      {
        ...
        justification:
          "75 GSM A4 Bond Paper Reams for End-Semester Question Papers & Answer Booklets.",
      ...
      .insert([
        {
          ...
          approver_role: "hod",
          status: "approved",
          comments: "Approved for urgent mid-term exams printing.",
        },
      ]);
  }
  ...
  const { data: rfq1 } = await supabase
    .from("rfqs")
    .insert([
      {
        ...
        notes:
          "Competitive solicitation for 10x AI GPU Workstations. Compliance with OEM warranty and on-site NBD support mandatory.",
    ...
    await supabase.from("quotation_lines").insert([
      {
        ...
        vendor_id: vApex,
        description:
          'Custom Intel Xeon AI Workstation (64GB DDR5, RTX 4080 16GB, 2TB Gen4, 27" 4K)',
        quantity: 10,
        unit: "units",
        unit_price: 117500,
        tax_amount: 211500,
        ...
        technical_remarks:
          "Fully compliant with 3-year comprehensive on-site warranty & NVIDIA AI Enterprise support.",
      ...
      },
      {
        ...
        vendor_id: vDell,
        description:
          'Dell Precision 5860 Tower Workstation (64GB ECC DDR5, RTX 4080 16GB, 2TB, 27" 4K)',
        quantity: 10,
        unit: "units",
        unit_price: 122000,
        tax_amount: 219600,
    ...
    .insert([
      {
        ...
        negotiation_notes:
          "Vendor Apex Tech Solutions Pvt Ltd agreed to 2% prompt settlement discount and 36-month NBD on-site warranty.",
        price_reasonableness_notes:
          "Lowest responsive bidder (L1). Unit rate of ₹1,17,500 is 2.1% lower than current institutional benchmark.",
    ...
    await supabase.from("cs_line_scores").insert([
      {
        ...
        total_score: 94.2,
        rank: 1,
        notes: "L1 Bidder, fully compliant.",
      },
      {
        cs_id: cs1.id,
        vendor_id: vDell,
        quoted_total: 1220000,
        price_score: 88.0,
        ...
        total_score: 90.4,
        rank: 2,
        notes: "L2 Bidder, longer delivery timeline.",
      },
      {
        cs_id: cs1.id,
        vendor_id: vLenovo,
        quoted_total: 1245000,
        price_score: 85.0,
    ...
    .insert([
      {
        ...
        type: "regular",
        scope_of_supply:
          "10x High Performance AI Workstations with 3-Year On-Site OEM Warranty & Installation.",
        price: 1175000,
        taxes: 211500,
        delivery_timeline: "14 calendar days from PO date",
        payment_terms:
          "100% upon delivery, installation, technical inspection and 3-way match verification.",
        status: "issued",
    ...
    await supabase.from("po_amendments").insert([
      {
        po_id: po1.id,
        changed_fields_json: {
          delivery_timeline: "Extended by 5 days due to customs clearance on GPU chipsets",
        },
  ...
  if (po1) {
      ...
      .insert([
        {
          ...
          items_json: [
            {
              item: "Intel Xeon AI Workstation",
              qty: 10,
              serial_range: "AIW-2026-001 to AIW-2026-010",
            },
          ],
          packages_count: 10,
    ...
    if (dc1) {
      const { data: grn1 } = await supabase
        .from("grns")
        .insert([
          {
        ...
        await supabase.from("grn_lines").insert([
          {
            ...
            unit_price: 117500,
            inspection_remarks:
              "Hardware diagnostic & GPU stress test benchmark completed successfully across all 10 nodes.",
          },
        ]);
      }
    }
  ...
  if (po1) {
    const { data: grnRecord } = await supabase
      .from("grns")
      .select("id")
      .eq("po_id", po1.id)
      .limit(1)
  ...
  await supabase.from("emergency_procurements").insert([
    {
      ...
      reason_standard_process_failed:
        "Heavy construction excavation damaged the primary 24-core single-mode fiber line, disconnecting central data center & university admissions portal.",
      is_post_facto: true,
      evp_approval_status: "approved",
      evp_approved_by: evpId,
      evp_approved_at: new Date().toISOString(),
      quotations_obtained_count: 2,
      price_reasonableness_note:
        "Emergency on-call industrial fiber splicing contractor engaged at standard PWD scheduled emergency rate.",
      vendor_id: vApex,
      financial_year: "2026-27",
    },
  ]);
  ...
  const vRatings = [
    {
      ...
      outcome: "preferred",
      notes:
        "Exceptional SLA response, flawless technical compliance, and competitive prompt-payment terms.",
  ...
  const tickets = [
    {
      ...
      issue_description:
        "Lab Node #04 in CSE AI Computing Lab requires RAM module diagnostic and thermal paste reapplication.",
      priority: "high",
      status: "in-progress",
      created_by: facultyId,
    ...
    },
    {
      ...
      issue_description:
        "Three-phase stabilizer in Workshop CNC bay experiencing voltage fluctuation during peak hours.",
      priority: "urgent",
      status: "pending",
      created_by: hodMechId,
    },
  ...
  for (const t of tickets) {
    const { data: insertedTkt } = await supabase
      .from("tickets")
      .upsert(t, { onConflict: "ticket_number" })
      .select("id")
      .single();
    if (insertedTkt) {
