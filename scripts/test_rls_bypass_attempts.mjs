...
const SUPABASE_URL = process.env.VITE_SUPABASE_URL || 'https://c--af0738c2-48d6-4b19-953d-357a854b7347-prod.lovable.cloud';
const SUPABASE_KEY = process.env.VITE_SUPABASE_PUBLISHABLE_KEY || 'sb_publishable_kdIaKgHivbo7PoNSzaYRMw_mmrwnaft';
...
console.log('Query: supabase.from(\'purchase_orders\').update({ status: \'approved\' }).eq(\'id\', \'po-e08f51a2-9b1c-4d3e-9081-7c98e1f51234\')');
try {
  ...
  if (res1.error || (res1.data && res1.data.length === 0)) {
    console.log('>>> VERDICT: DIRECT CLIENT MUTATION BLOCKED BY POSTGRES RLS (0 rows modified / permission denied).');
  } else {
...
console.log('Query: supabase.from(\'emergency_procurements\').update({ evp_approval_status: \'approved\' }).eq(\'id\', \'ep-771a82d3-11ef-49c0-9a45-667123abcdef\')');
try {
...
console.log('[CALL 3] Session Role: "procurement_officer" attempting direct Vendor status empanelment');
console.log('Query: supabase.from(\'vendors\').update({ status: \'empanelled\' }).eq(\'id\', \'v-339c01fa-4e89-42b1-b921-998811223344\')');
try {
