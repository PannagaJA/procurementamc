...
async function seed() {
    ...
    .insert([
      {
        ...
        justification:
          "Required for newly introduced M.Tech AI/ML curriculum & GPU compute laboratory.",
      },
    ])
    .select()
    .single();
    ...
    await supabase.from("pr_line_items").insert([
      {
        ...
        specification:
          "Intel Xeon Silver 4410Y, 64GB DDR5 ECC, 2TB NVMe Gen4, NVIDIA RTX 4080 16GB, 27-inch 4K Monitor",
        quantity: 10,
  ...
  if (pr1) {
    ...
    if (rfq) {
      ...
      const { data: qLenovo } = await supabase
        .from("quotations")
        .insert([
          {
            rfq_id: rfq.id,
        ...
        .insert([
          {
            ...
            recommendation_rationale:
              "Lowest compliant bid (L1). Meets all OEM GPU & RAM specifications with 3-year onsite warranty.",
              ...
              .insert([
                {
                  ...
                  remarks:
                    "All 10 systems unboxed, benchmarked and accepted in good working condition.",
                },
              ])
              .select()
              .single();
  ...
  await supabase.from("emergency_procurements").insert([
    {
      ...
      description:
        "Catastrophic battery burst during power surge threatening central campus datacenter.",
