...
const guards = {
  assertPrApproved(pr) {
    ...
    if (pr.status !== 'approved') {
      throw new ProcurementRuleError('PR_NOT_APPROVED', `Cannot create RFQ or PO. PR #${pr.pr_number || pr.id} is in status "${pr.status}". Must be "approved".`);
    }
    return true;
  },
  ...
  assertVendorEmpanelled(vendor) {
    ...
    if (vendor.status === 'blacklisted' || vendor.status === 'debarred') {
      throw new ProcurementRuleError('VENDOR_BLACKLISTED', `Vendor "${vendor.name}" is debarred/blacklisted (SOP §8.2, §13).`);
    }
    if (vendor.status === 'suspended') {
      throw new ProcurementRuleError('VENDOR_SUSPENDED', `Vendor "${vendor.name}" is suspended and cannot be engaged (SOP §8.2).`);
    }
    if (vendor.status !== 'approved' && vendor.status !== 'empanelled') {
      throw new ProcurementRuleError('VENDOR_NOT_EMPANELLED', `Vendor "${vendor.name}" is not empanelled (Status: "${vendor.status}").`);
    }
    return true;
  },

  assertNoPoSplitting({ pr, newPoAmount, existingPosOnPr = [], recentVendorPos = [], deviationApproved = false }) {
    ...
    if (pr && pr.estimated_cost && cumulativeOnPr > Number(pr.estimated_cost) * 1.05) { // 5% max variance
  ...
  assertThreeWayMatch({ invoice, po, grn, isServicePo, completionCertUrl, tolerance = 0.01 }) {
    if (isServicePo) {
      if (!completionCertUrl) {
        throw new ProcurementRuleError('SERVICE_CERT_MISSING', 'Work Completion Certificate is required for Service POs in lieu of GRN (SOP §I2).');
      }
      if (Math.abs(Number(invoice.invoice_amount) - Number(po.total_value)) > tolerance) {
        throw new ProcurementRuleError('SERVICE_INVOICE_MISMATCH', `Service invoice amount ₹${invoice.invoice_amount} differs from PO total ₹${po.total_value}.`);
      }
      return true;
    }

    if (!grn) throw new ProcurementRuleError('GRN_REQUIRED', 'Approved Goods Receipt Note (GRN) is required for physical goods matching (SOP §8.7).');
    if (grn.status !== 'accepted') {
      throw new ProcurementRuleError('GRN_NOT_ACCEPTED', `GRN #${grn.grn_number} status is "${grn.status}". Must be "accepted".`);
    }
    ...
    if (invAmt > grnVal + tolerance) {
      throw new ProcurementRuleError('INVOICE_EXCEEDS_GRN', `Invoice amount (₹${invAmt}) exceeds GRN accepted value (₹${grnVal}).`);
    }
    if (invAmt > poVal + tolerance) {
      throw new ProcurementRuleError('INVOICE_EXCEEDS_PO', `Invoice amount (₹${invAmt}) exceeds PO total value (₹${poVal}).`);
    }

    return true;
  },

  assertEmergencyCapNotExceeded({ runningTotal, newAmount, capLimit = 1000000, financialYear = '2026-27' }) {
...
try {
  ...
  assert.ok(rHodMonthCap.approverRole === 'principal' || rHodMonthCap.approverRole === 'procurement_officer');
  pass('Small Value ₹1,500 with ₹4,000 MTD spend (Total ₹5,500 > ₹5,000 HOD month cap) -> Escalates to Principal tier');
  ...
  pass('Small Value ₹3,000 with ₹28,000 MTD spend (Total ₹31,000 > ₹30,000 month cap) -> Escalates to EVP');
} catch (e) {
...
try {
  ...
  pass('Routine Consumable ₹25,000 (Total ₹75,000 <= ₹1,00,000 monthly cap) -> Procurement Officer');
...
const assetCategories = ['equipment_asset', 'software', 'academic_research', 'services_amc', 'maintenance'];
...
try {
  ...
  assert.throws(
    ...
    (err) => err.code === 'PR_NOT_APPROVED' || err.message.includes('PR_NOT_APPROVED') || err.message.includes('draft')
  );
  assert.throws(
...
try {
  assert.strictEqual(guards.assertVendorEmpanelled({ id: 'v-1', name: 'Apex Ltd', status: 'empanelled' }), true);
  assert.strictEqual(guards.assertVendorEmpanelled({ id: 'v-2', name: 'Zenith Corp', status: 'approved' }), true);
...
try {
  ...
  assert.strictEqual(guards.assertNoPoSplitting({ pr, newPoAmount: 7000, existingPosOnPr: existingPos, recentVendorPos: [] }), true);
  pass('PO within cumulative PR budget is permitted.');

  // Blocked exceeding PR budget (12k + 10k = 22k > 20k * 1.05)
  assert.throws(
    () => guards.assertNoPoSplitting({ pr, newPoAmount: 10000, existingPosOnPr: existingPos, recentVendorPos: [] }),
    (err) => err.code === 'PO_SPLITTING_PR_EXCEEDED' || err.message.includes('exceeds approved PR budget')
  );
  ...
  assert.throws(
    () => guards.assertNoPoSplitting({ pr, newPoAmount: 7000, existingPosOnPr: [], recentVendorPos: recentPos, deviationApproved: false }),
    (err) => err.code === 'PO_SPLITTING_30D_WINDOW' || err.message.includes('POTENTIAL PO-SPLITTING DETECTED')
  );
  ...
  assert.strictEqual(guards.assertNoPoSplitting({ pr, newPoAmount: 7000, existingPosOnPr: [], recentVendorPos: recentPos, deviationApproved: true }), true);
...
try {
  ...
  assert.strictEqual(guards.assertThreeWayMatch({ invoice: validInvoice, po, grn, isServicePo: false }), true);
  ...
  assert.strictEqual(guards.assertThreeWayMatch({ invoice: tolerantInvoice, po, grn, isServicePo: false, tolerance: 0.01 }), true);
  ...
  assert.throws(
    ...
    (err) => err.code === 'INVOICE_EXCEEDS_GRN' || err.message.includes('exceeds GRN accepted value')
  );
  ...
  assert.strictEqual(guards.assertThreeWayMatch({ invoice: serviceInv, po: servicePo, grn: null, isServicePo: true, completionCertUrl: 'https://docs.amc.org/cert.pdf' }), true);
  pass('Service PO with valid Completion Certificate passes in lieu of GRN (§I2).');

  assert.throws(
    () => guards.assertThreeWayMatch({ invoice: serviceInv, po: servicePo, grn: null, isServicePo: true, completionCertUrl: null }),
    (err) => err.code === 'SERVICE_CERT_MISSING' || err.message.includes('Work Completion Certificate is required')
  );
...
try {
  ...
  assert.strictEqual(guards.assertEmergencyCapNotExceeded({ runningTotal: 900000, newAmount: 100000, capLimit: 1000000, financialYear: '2026-27' }), true);
  ...
  assert.throws(
    () => guards.assertEmergencyCapNotExceeded({ runningTotal: 900000, newAmount: 100001, capLimit: 1000000, financialYear: '2026-27' }),
  ...
  assert.strictEqual(guards.assertEmergencyCapNotExceeded({ runningTotal: 0, newAmount: 250000, capLimit: 1000000, financialYear: '2027-28' }), true);
...
try {
  ...
  const sPref = computeWeightedScore({ sectionA: 95, sectionB: 90, sectionC: 90, sectionD: 85, sectionE: 80, sectionF: 85 });
  assert.strictEqual(deriveOutcome(sPref), 'preferred');
  pass(`Score ${sPref} -> Band 1: PREFERRED (>= 85)`);

  // Band 2: Active (Score 70 - 84.99)
  const sActive = computeWeightedScore({ sectionA: 75, sectionB: 75, sectionC: 70, sectionD: 75, sectionE: 70, sectionF: 75 });
  ...
  const sNotice = computeWeightedScore({ sectionA: 60, sectionB: 60, sectionC: 60, sectionD: 60, sectionE: 60, sectionF: 60 });
  ...
  const sSusp = computeWeightedScore({ sectionA: 45, sectionB: 50, sectionC: 45, sectionD: 50, sectionE: 40, sectionF: 45 });
  assert.strictEqual(deriveOutcome(sSusp), 'suspended');
  pass(`Score ${sSusp} -> Band 4: SUSPENDED (40 - 54.99)`);

  // Band 5: Debarred (Score < 40)
  const sDebar = computeWeightedScore({ sectionA: 30, sectionB: 35, sectionC: 30, sectionD: 25, sectionE: 30, sectionF: 20 });
  ...
  const sNoE = computeWeightedScore({ sectionA: 90, sectionB: 80, sectionC: 80, sectionD: 70, sectionE: null, sectionF: 80 });
...
try {
  ...
  const pr = { id: 'pr-101', pr_number: 'PR-2026-0101', category: 'equipment_asset', estimated_cost: 1200000, status: 'draft' };
  ...
  const rfq = { id: 'rfq-201', rfq_number: 'RFQ-2026-0201', pr_id: pr.id, min_quotations: 3, status: 'sent' };
  ...
  pass('Cycle A [Step 6]: Delivery Challan recorded; Gate Security & Tech Acceptance signed off -> GRN-2026-0601 accepted.');
  ...
  pass('Cycle A [Step 7]: Invoice verified via 3-Way Match (PO: ₹11.4L == GRN: ₹11.4L == Inv: ₹11.4L) -> Approved by Finance.');

  // Step 8: Payment Disbursement & PO Final Closure
  const payment = { id: 'pay-801', invoice_id: invoice.id, amount: 1140000, external_ref: 'NEFT-SBIN20260918001' };
  invoice.status = 'paid';
  po.status = 'closed';
  pass('Cycle A [Step 8]: Payment of ₹11,40,000 disbursed (UTR logged) -> PO status updated to "closed".');
} catch (e) {
...
try {
  ...
  const emergencyReq = {
    ...
    reason_standard_process_failed: 'Imminent total campus power loss due to oil leak; 7-day RFQ impossible.',
  ...
  guards.assertEmergencyCapNotExceeded({ runningTotal: 650000, newAmount: emergencyReq.estimated_cost, capLimit: 1000000 });
  pass('Cycle B [Step 1-2]: Emergency Request EP-2026-0901 (₹1.75L) within FY headroom (₹6.50L + ₹1.75L = ₹8.25L <= ₹10L).');
  ...
  pass(`Cycle B [Step 3]: Post-facto ratification completed within ${diffHours.toFixed(1)} hours (<= 48h window) by EVP.`);
} catch (e) {
...
try {
  ...
  const lowScores = { sectionA: 25, sectionB: 20, sectionC: 30, sectionD: 25, sectionE: 20, sectionF: 20 };
  ...
  pass(`Cycle C [Step 2]: Performance rating completed: Weighted Score = ${weightedScore} (< 40.0) -> Outcome: DEBARRED.`);
  ...
  pass('Cycle C [Step 4]: Blacklisted vendor is strictly blocked from participating in any new RFQ.');
} catch (e) {
