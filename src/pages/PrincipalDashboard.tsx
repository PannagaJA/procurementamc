...
const PrincipalDashboard = () => {
    ...
    try {
      const [pendingRes, inProgRes, approvedRes, rejectedRes] = await Promise.all([
        ...
        supabase
          .from("tickets")
          .select("id", { head: true, count: "exact" })
          .in("status", ["procure-approved", "service-approved"]),
        supabase
          .from("tickets")
          .select("id", { head: true, count: "exact" })
          .in("status", ["procure-rejected", "service-rejected"]),
      ]);
      ...
      const counts = await Promise.all(
        Object.entries(SLA_RULES).map(async ([priority, rule]) => {
  ...
  const fetchTickets = useCallback(async () => {
    ...
    try {
      ...
      if (filters.search) {
        const esc = String(filters.search || "")
          .trim()
          .replace(/%/g, "\\%")
          .replace(/,/g, "");
        if (esc !== "") {
      ...
      const filtered = filters.overdueOnly
        ? computed.filter(
            (t: any) => (t.sla?.respondDaysOver || 0) > 0 || (t.sla?.resolveDaysOver || 0) > 0,
          )
        : computed;
      setTickets(filtered);
      setTotal(count || 0);
    ...
    try {
        ...
        .insert([
          {
            ...
            status_to: "procure-approved",
            admin_notes: principalNotes || "Approved by Principal",
            updated_by: userId,
          },
        ]);

      await logAudit("approve_ticket", userId, {
        ticket_id: ticket.id,
      ...
      try {
        ...
        await sendNotification(
          ownerId,
          `Your request ${ticket.ticket_number} was approved by Principal`,
          { ticket_id: ticket.id },
        );
      } catch (e) {
        ...
        if (adminIds.length > 0) {
          ...
          await Promise.all(
            adminIds.map((aid: string) =>
              sendNotification(aid, `Request ${ticket.ticket_number} approved by Principal`, {
                ticket_id: ticket.id,
              }),
            ),
          );
  ...
  const rejectTicket = async (ticket: any) => {
    try {
      const reason =
        window.prompt("Reason for rejection (visible to requester):", "") ||
        ...
        .insert([
          {
            ticket_id: ticket.id,
            status_from: ticket.status,
            status_to: "procure-rejected",
            admin_notes: reason,
            updated_by: userId,
          },
        ]);
      await logAudit("reject_ticket", userId, {
        ticket_id: ticket.id,
        ticket_number: ticket.ticket_number,
        reason,
      });
      ...
      try {
        ...
        await sendNotification(
          ownerId,
          `Your request ${ticket.ticket_number} was rejected by Principal: ${reason}`,
          { ticket_id: ticket.id },
        );
      } catch (e) {
        console.warn("notify owner failed", e);
  ...
  useEffect(() => {
    ...
    if (remoteError) {
      ...
      toast({
        ...
        description: "Supabase appears unreachable — data fetches paused.",
        variant: "destructive",
      });
      return;
    }
  ...
  const performAction = async (
    type: "approve" | "reject" | "escalate" | "assign",
    ticket: any,
    note: string | null,
    payload?: any,
  ) => {
    try {
      if (type === "approve") {
        const newStatus =
          ticket.issue_category === "procure" ? "procure-approved" : "service-approved";
          ...
          .insert([
            {
              ...
              status_from: ticket.status,
              status_to: newStatus,
              admin_notes: note || "Approved by Principal",
              updated_by: userId,
            },
          ]);
        ...
        const newStatus =
          ticket.issue_category === "procure" ? "procure-rejected" : "service-rejected";
        const { error } = await supabase
          .from("tickets")
          .update({ status: newStatus })
          .eq("id", ticket.id);
          ...
          .insert([
            {
              ...
              admin_notes: note || "Escalated by Principal",
              updated_by: userId,
            },
          ]);
        await logAudit("principal_escalate", userId, {
          ticket_id: ticket.id,
          ticket_number: ticket.ticket_number,
        });
      } else if (type === "assign") {
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                <Button size="sm" onClick={() => navigate({ to: "/principal/approvals" })}>
                  Approve Requests
                </Button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-4">
              <div className="p-4 rounded-xl bg-gradient-to-r from-yellow-50 to-yellow-100 shadow-sm">
                <div className="text-xs text-muted-foreground flex items-center gap-2">
                  <CalendarIcon className="w-4 h-4 text-yellow-600" /> Pending Approval
                </div>
              ...
              <div className="p-4 rounded-xl bg-gradient-to-r from-blue-50 to-sky-50 shadow-sm">
                <div className="text-xs text-muted-foreground flex items-center gap-2">
                  <Activity className="w-4 h-4 text-sky-600" /> In Progress
                </div>
                  ...
                  <label className="flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        setFilters((p) => ({
                          ...p,
                          overdueOnly: (e.target as HTMLInputElement).checked,
                        }))
                      }
                    />{" "}
                    Overdue only
                      ...
                      {tickets.map((t) => (
                        <tr key={t.id} className="border-t">
                          <td className="px-3 py-2 align-middle">
                            <div className="truncate max-w-full">{t.ticket_number}</div>
                          </td>
                          <td className="px-3 py-2 align-middle">
                            <div className="truncate max-w-full">
                              {(() => {
                                const found = departments.find(
                                  (d) =>
                                    String(d.id) === String(t.department) ||
                                    String(d.id) === String(t.department_id),
                                );
                                return found
                                  ? found.name
                          ...
                          <td className="px-3 py-2 align-middle">
                            <div className="truncate max-w-full">{t.name}</div>
                          </td>
                          <td className="px-3 py-2 align-middle">
                            <div className="inline-block">
                              <Badge className={getPriorityColor(t.priority)}>
                                {String(t.priority || "")
                                  .charAt(0)
                                  .toUpperCase() + String(t.priority || "").slice(1)}
                              </Badge>
                            </div>
                          </td>
                          <td className="px-3 py-2 align-middle">
                            <div className="truncate max-w-full">
                              {t.sla?.respondDaysOver ? (
                                <Badge className="bg-rose-100 text-rose-800">{`${t.sla.respondDaysOver}d overdue`}</Badge>
                              ) : (
                                <span className="text-sm text-muted-foreground">
                                  {t.sla?.respondByDisplay}
                                </span>
                              )}
                            </div>
                          </td>
                          <td className="px-3 py-2 align-middle text-right">
                            <div className="inline-flex items-center justify-end gap-2">
                              <Button size="sm" onClick={() => openTicketDetails(t)}>
                                View
                              </Button>
                              <Button
                                size="sm"
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <div className="font-medium">
                      <Badge className={getStatusColor(viewTicket.status)}>
                        {String(viewTicket.status).replace("-", " ").toUpperCase()}
                      </Badge>
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="font-medium">
                      <Badge className={getPriorityColor(viewTicket.priority)}>
                        {String(viewTicket.priority || "").toUpperCase()}
                      </Badge>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    placeholder="Optional note for approve/reject/escalate"
                  />
                </div>
                        ...
                        <div key={u.id} className="p-3 border rounded bg-gray-50">
                          <div className="flex items-center justify-between">
                            <div className="text-sm font-medium">
                              {String(u.status_from || "").toUpperCase()} →{" "}
                              {String(u.status_to || "").toUpperCase()}
                            </div>
                            <div className="text-xs text-muted-foreground">
                              {u.created_at ? format(new Date(u.created_at), "PP p") : ""}
                            </div>
                          </div>
                ...
                <div className="flex justify-end gap-2">
                  ...
                  <Button disabled title="Actions disabled in view mode">
                    Escalate
                  </Button>
                  <Button variant="destructive" disabled title="Actions disabled in view mode">
                    Reject
                  </Button>
                  <Button disabled title="Actions disabled in view mode">
                    Approve
