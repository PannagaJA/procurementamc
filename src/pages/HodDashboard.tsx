...
const HodDashboard = () => {
  ...
  const fetchDepartmentForUser = useCallback(async () => {
    try {
      ...
      toast({
        title: "Error",
        description: "Failed to load HOD department",
        variant: "destructive",
      });
    }
  }, [toast, departmentId]);
  ...
  const fetchTickets = useCallback(async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: "Failed to load tickets for your department",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
    ...
    try {
      ...
      const [pendingRes, inProgRes, approvedRes, rejectedRes] = await Promise.all([
        ...
        supabase
          ...
          .select("id", { head: true, count: "exact" })
          .eq("department", departmentName)
          .eq("status", "procure-approved"),
        supabase
          .from("tickets")
          .select("id", { head: true, count: "exact" })
          .eq("department", departmentName)
          .eq("status", "procure-rejected"),
      ]);
      setMetrics({
  ...
  const fetchInventory = useCallback(async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: "Failed to load inventory for your department",
        variant: "destructive",
      });
    } finally {
  ...
  const computeSLAForTicket = (ticket: any) => {
    try {
      ...
      }[ticket.priority as "high" | "medium" | "low"] || { respondHours: 24, resolveHours: 168 };
      ...
      ticket.sla = {
        respondByDisplay: respondBy.toISOString(),
            ...
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="flex items-center gap-2">
                  <Activity className="w-5 h-5 text-emerald-600" /> HOD Dashboard
                </CardTitle>
                <p className="text-sm text-muted-foreground">
                  Tickets and stats for {departmentName || "your department"}
                </p>
              </div>
                  ...
                  {tickets.map((t) => (
                    <tr key={t.id} className="border-t">
                      <td className="px-3 py-2 align-middle">
                        <div className="font-mono">{t.ticket_number}</div>
                      </td>
                      <td className="px-3 py-2 align-middle">
                        <div className="truncate max-w-full">{t.issue_category}</div>
                      </td>
                      <td className="px-3 py-2 align-middle">
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="flex items-center gap-2">
                    <Activity className="w-5 h-5 text-emerald-600" /> Inventory
                  </CardTitle>
                  <p className="text-sm text-muted-foreground">
                    Items for {departmentName || "your department"}
                  </p>
                </div>
              </div>
            </CardHeader>
                    ...
                    <tbody>
                      {inventoryItems.map((it) => (
                        <tr key={it.id} className="border-t">
                          ...
                          <td className="px-2 py-2 align-middle">
                            <div className="truncate max-w-full">{it.department || "-"}</div>
                          </td>
                          <td className="px-2 py-2 align-middle">
                            <div className="truncate max-w-full">
                            ...
                            <Button
                              ...
                              onClick={() =>
                                navigate({
                                  ...
                                  state: { from: "hod-dashboard" } as any,
                                })
                              }
                            >
                              View
              ...
              <div className="space-y-4">
                ...
                {viewTicket.issue_category === "procure" && (
                  <div>
                    ...
                    <div className="mt-1">
                      {Object.entries(formatProcureDescription(viewTicket.issue_description)).map(
                        ([k, v]) => (
                      ...
                      viewUpdates.map((u) => (
                        <div key={u.id} className="p-2 border rounded">
                          ...
                          <div className="mt-1 whitespace-pre-wrap">
                            {u.notes || u.description || u.admin_notes}
                          </div>
                          {u.status_to ? (
                            <div className="mt-1">
                              <Badge className={getStatusColor(u.status_to)}>
                                {String(u.status_to).replace("-", " ").toUpperCase()}
                              </Badge>
                            </div>
                          ) : null}
                        </div>
