...
interface Ticket {
  ...
  status: 'pending' | 'in-progress' | 'waiting-for-user'  | 'completed' | 'procure-in-progress' | 'procure-completed' | 'procure-approved' | 'procure-rejected' | 'service-approved' | 'service-rejected';
  created_at: string;
  updated_at: string;
  ticket_updates: {
      ...
      return (
        <div className="space-y-2">
          ...
          {procureData['Approval Letter'] && (
            <div>
              <strong>Approval Letter:</strong> <a href={procureData['Approval Letter']} target="_blank" rel="noopener noreferrer" className="text-primary hover:text-primary/80 hover:underline">View Document</a>
            </div>
          )}
        </div>
      );
      ...
      <Card>
        <CardHeader>
          <div className="flex justify-between items-center">
            ...
            <Button onClick={() => navigate({ to: '/raise-ticket' })}>
              Raise New Ticket
            </Button>
          </div>
        </CardHeader>
                      ...
                      <div>
                        <div className="flex justify-end mb-2 space-x-2">
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => setOpenUpdateTicketId(prev => prev === ticket.id ? null : ticket.id)}
                            aria-expanded={openUpdateTicketId === ticket.id}
                            aria-controls={`admin-updates-${ticket.id}`}
                          >
                            <EyeIcon className="w-4 h-4 mr-2" />
                            {openUpdateTicketId === ticket.id ? 'Hide Admin Updates' : 'View Admin Updates'}
                          </Button>

                          <Dialog open={showDialog && viewTicket?.id === ticket.id} onOpenChange={(open) => { if (!open) { setShowDialog(false); setViewTicket(null); } }}>
                            <DialogTrigger asChild>
                              <Button
                                variant="outline"
                                size="sm"
                                onClick={() => { setViewTicket(ticket); setShowDialog(true); }}
                              >
                                View Details
                              </Button>
                            </DialogTrigger>
                              ...
                              <div className="mt-2 space-y-3">
                                ...
                                {viewTicket && viewTicket.issue_category === 'procure' && (
                                  <div>
                                    ...
                                    <p className="text-sm text-muted-foreground">{formatProcureDescription(viewTicket.issue_description)['Quantity']}</p>
                                    <p className="text-sm font-medium mt-2">Specifications</p>
                                    <p className="text-sm text-muted-foreground">{formatProcureDescription(viewTicket.issue_description)['Specifications']}</p>
                                    <p className="text-sm font-medium mt-2">Estimated Cost</p>
                                    <p className="text-sm text-muted-foreground">₹{formatProcureDescription(viewTicket.issue_description)['Estimated Cost']}</p>
                                    <p className="text-sm font-medium mt-2">Justification</p>
                                    <p className="text-sm text-muted-foreground">{formatProcureDescription(viewTicket.issue_description)['Justification']}</p>
                                  </div>
                                )}

                                <div>
                                  <p className="text-sm font-medium">SLA</p>
                                  <p className="text-sm text-muted-foreground">{viewTicket?.priority ? `${viewTicket.priority.charAt(0).toUpperCase() + viewTicket.priority.slice(1)}` : '—'}</p>
                                </div>

                                <div>
                                  <p className="text-sm font-medium">Created</p>
                                  <p className="text-sm text-muted-foreground">{viewTicket ? new Date(viewTicket.created_at).toLocaleString() : ''}</p>
                                </div>
                              </div>
                            </DialogContent>
                          </Dialog>
                        ...
                        {openUpdateTicketId === ticket.id && (
                          <div id={`admin-updates-${ticket.id}`} className="border rounded-lg p-4 bg-muted/30">
                              ...
                              {ticket.ticket_updates.map((update, index) => (
                                <div key={index} className="text-sm">
                                  {update.status_from && update.status_to && (
                                    <p>
                                      <span className="font-medium">Status:</span> {update.status_from.replace('-', ' ')} → {update.status_to.replace('-', ' ')}
                                    </p>
                                  )}
                                  {update.admin_notes && (
                                    <p>
                                      <span className="font-medium">Note:</span> {update.admin_notes}
                                    </p>
                                  )}
