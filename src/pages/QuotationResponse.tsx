...
const responseSchema = z.object({
  ...
  contactPhone: z.string().min(5, "Contact phone is required").refine((v) => {
    // basic phone validation: allow digits, spaces, +, -, (, )
    return /^[0-9+()\s-]{5,}$/.test(v)
  }, "Enter a valid phone number"),
  paymentTerms: z.string().min(1, "Payment terms are required"),
  additionalNotes: z.string().optional(),
  quotationValidity: z.string().min(1, "Quotation validity date is required").refine((val) => !isNaN(Date.parse(val)), "Enter a valid date"),
  termsAccepted: z.boolean().refine((v) => v === true, "You must accept the Terms & Conditions to submit"),
})
    ...
    <div className="container mx-auto py-8 max-w-4xl">
      <Card>
        <CardHeader className={`${
          isExpired ? 'bg-red-50' : hasResponded ? 'bg-green-50' : 'bg-gradient-to-r from-blue-50 to-indigo-50'
        }`}>
          <div className="flex items-center gap-4">
            ...
            <div>
              <CardTitle className="flex items-center gap-2">
                ...
                {hasResponded && <span className="text-green-600 text-sm font-normal">(Response Submitted)</span>}
              </CardTitle>
              <CardDescription>
                {isExpired
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="text-lg font-semibold text-green-600">₹{existingResponse?.total_amount || 'N/A'}</p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {extractFromDescription(existingResponse?.description || '', 'Delivery Time') || 'Not specified'}
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {extractFromDescription(existingResponse?.description || '', 'Contact Person') || 'Not specified'}
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {(() => {
                        const phone = extractFromDescription(existingResponse?.description || '', 'Contact Phone');
                        if (!phone) return 'Not specified';
                        const tel = phone.replace(/[^0-9+]/g, '');
                        return <a className="text-blue-600 underline" href={`tel:${tel}`}>{phone}</a>;
                      })()}
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {extractFromDescription(existingResponse?.description || '', 'Payment Terms') || 'Not specified'}
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="mt-1">
                    <Badge variant={
                      quotation?.admin_status === 'accepted' ? 'default' :
                      quotation?.admin_status === 'rejected' ? 'destructive' : 'secondary'
                    } className="text-sm px-3 py-1">
                      {quotation?.admin_status ? quotation.admin_status.charAt(0).toUpperCase() + quotation.admin_status.slice(1) : 'Unknown'}
                    </Badge>
                    </div>
                    {quotation?.admin_status === 'accepted' && (
                      <p className="text-sm text-green-600 mt-2">✅ Your quotation has been accepted!</p>
                    )}
                    {quotation?.admin_status === 'rejected' && (
                      <p className="text-sm text-red-600 mt-2">❌ Your quotation was not selected.</p>
                    )}
                    {quotation?.admin_status === 'pending' && (
                      <p className="text-sm text-blue-600 mt-2">⏳ Your quotation is under review.</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {existingResponse?.submitted_at ? new Date(existingResponse.submitted_at).toLocaleDateString() + ' at ' + new Date(existingResponse.submitted_at).toLocaleTimeString() : 'Unknown'}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="border-t pt-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{quotation.product_name} • {quotation.quantity}</p>
                </div>
                <div>
                  <label className="text-sm font-medium">Company</label>
                  <p className="text-sm text-muted-foreground">{quotation.company_email || 'N/A'}</p>
                </div>
                <div>
                  <label className="text-sm font-medium">Last Reply Date</label>
                  <p className="text-sm text-muted-foreground">{quotation.last_reply_date ? new Date(quotation.last_reply_date).toLocaleString() : 'N/A'}</p>
                </div>
                <div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <div className="bg-gray-50 p-3 rounded mt-1">
                          <p className="text-sm whitespace-pre-line">{existingResponse.description}</p>
                        </div>
                      </div>
                    </div>
                  </>
                ) : (
                  <div className="text-sm text-muted-foreground">No response available yet for this quotation.</div>
                )}
              </div>
            </div>
          ) : !isExpired ? (
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>
                          Include product specifications, quality standards, and any special features
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                    ...
                    render={({ field }) => (
                      <FormItem className="flex items-start gap-2">
                        ...
                        <div className="text-sm">
                          <FormLabel className="font-medium">I accept the Terms & Conditions *</FormLabel>
                          <div className="text-muted-foreground">
                            <Dialog>
                              <DialogTrigger asChild>
                                <button type="button" className="text-blue-600 underline ml-2">Read Terms & Conditions</button>
                              </DialogTrigger>
                              <DialogContent>
                                <DialogHeader>
                                  <DialogTitle>Terms & Conditions</DialogTitle>
                                  <DialogDescription>
                                    Please read the supplier terms and conditions. By checking the box you accept these terms.
                                  </DialogDescription>
                                </DialogHeader>
                                <div className="mt-4 text-sm">
                                  <p>By submitting this quotation response you agree to the terms provided. (Provide full T&C text here.)</p>
                                </div>
                              </DialogContent>
                            </Dialog>
                          </div>
                  ...
                  <div className="bg-blue-50 p-4 rounded-lg">
                    <p className="text-sm text-blue-800">
                      <strong>Important:</strong> By submitting this quotation, you agree to honor the quoted price and terms for 30 days from the response deadline, unless otherwise agreed upon.
                    </p>
                  </div>
