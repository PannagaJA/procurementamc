...
const PrincipalApprovals = () => {
  ...
  const fetchPending = useCallback(async () => {
    ...
    try {
      ...
      let query = supabase
        ...
        .in('status', ['pending_principal', 'procure-approved', 'procure-rejected', 'service-approved', 'service-rejected'])
  ...
  const performAction = async (type: 'approve' | 'reject', t: any, note: string | null) => {
    try {
      const adminNote = (note !== null && note !== undefined) ? note : (type === 'approve' ? 'Approved by Principal' : 'Rejected by Principal');

      if (type === 'approve') {
        const newStatus = t.issue_category === 'procure' ? 'procure-approved' : 'service-approved';
        const { error: updErr } = await supabase.from('tickets').update({ status: newStatus }).eq('id', t.id);
        if (updErr) throw updErr;
        await supabase.from('ticket_updates').insert([{ ticket_id: t.id, status_from: t.status, status_to: newStatus, admin_notes: adminNote, updated_by: userId }]);
        await logAudit('principal_approve', userId, { ticket_id: t.id, ticket_number: t.ticket_number });
        ...
        try {
          ...
          if (adminIds.length) {
            ...
            await Promise.all(adminIds.map((aid: string) => sendNotification(aid, `Request ${t.ticket_number} approved by Principal`, { ticket_id: t.id })));
          }
        ...
        const { error: updErr } = await supabase.from('tickets').update({ status: newStatus }).eq('id', t.id);
        if (updErr) throw updErr;
        await supabase.from('ticket_updates').insert([{ ticket_id: t.id, status_from: t.status, status_to: newStatus, admin_notes: reason, updated_by: userId }]);
        await logAudit('principal_reject', userId, { ticket_id: t.id, ticket_number: t.ticket_number, reason });
        try {
          const { default: sendNotification } = await import('@/lib/notify');
          await sendNotification(t.created_by, `Your request ${t.ticket_number} was rejected by Principal: ${reason}`, { ticket_id: t.id });
  ...
  if (loading) return (
    <Layout>
      <div className="flex items-center justify-center min-h-screen"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-gray-900"></div></div>
    </Layout>
  );
  ...
  return (
    <>
                    ...
                    {requests.map(r => (
                      <tr key={r.id} className="border-t">
                        ...
                        <td className="px-3 py-2">
                          <Button size="sm" onClick={() => { setViewRequest(computeSLAForTicket(r)); }}>View</Button>
                          {r.status === 'pending_principal' ? (
                            <>
                              <Button size="sm" className="ml-2" onClick={() => openActionDialog('approve', r)}>Approve</Button>
                              <Button size="sm" variant="destructive" className="ml-2" onClick={() => openActionDialog('reject', r)}>Reject</Button>
                            </>
                          ) : (r.status === 'procure-approved' || r.status === 'service-approved') ? (
                            <Button size="sm" className="ml-2 bg-green-100 text-green-800" disabled>Approved</Button>
                          ) : (r.status === 'procure-rejected' || r.status === 'service-rejected') ? (
                            <Button size="sm" className="ml-2 bg-red-100 text-red-800" disabled>Rejected</Button>
                          ) : null}
                        </td>
                      </tr>
                    ))}
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <div className="font-medium">{String(viewRequest.priority || '').charAt(0).toUpperCase() + String(viewRequest.priority || '').slice(1)}</div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="font-medium">{viewRequest.contact_number || viewRequest.phone || '-'}</div>
                </div>
                <div>
              ...
              <div className="text-sm mt-2">
                ...
                <div className="mt-1">{viewRequest.issue_category === 'procure' ? renderTicketDescription(viewRequest) : <div className="whitespace-pre-wrap">{viewRequest.issue_description}</div>}</div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <div className="text-xs text-muted-foreground">Created</div>
                  <div className="font-medium">{viewRequest.created_at ? format(new Date(viewRequest.created_at), 'PP p') : '-'}</div>
                </div>
                <div>
                  <div className="text-xs text-muted-foreground">Updated</div>
                  <div className="font-medium">{viewRequest.updated_at ? format(new Date(viewRequest.updated_at), 'PP p') : '-'}</div>
                </div>
                <div>
                  <div className="text-xs text-muted-foreground">Attachments</div>
                  <div className="font-medium">{(viewRequest.issue_category === 'procure' && viewRequest.issue_description && String(viewRequest.issue_description).includes('Approval Letter')) ? 'Approval letter attached' : (viewRequest.attachment_url ? <a href={viewRequest.attachment_url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">View</a> : '-')}</div>
                </div>
              </div>
      ...
      <Dialog open={actionDialogOpen} onOpenChange={(open) => { if (!open) { setActionDialogOpen(false); setActionTarget(null); setActionType(null); } }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{actionType === 'approve' ? 'Approve Request' : 'Reject Request'}</DialogTitle>
            <DialogDescription>{actionType === 'approve' ? 'Add an optional note for approval (visible to Admin/HOD).' : 'Provide a reason for rejection (visible to HOD).'}</DialogDescription>
          </DialogHeader>
          ...
          {actionTarget ? (
            <div className="space-y-4">
              ...
              <div>
                <Label>Note</Label>
                <Textarea value={actionNote} onChange={(e) => setActionNote((e.target as HTMLTextAreaElement).value)} placeholder={actionType === 'approve' ? 'Optional approval note' : 'Reason for rejection'} />
              </div>

              <div className="flex justify-end gap-2">
                <Button variant="outline" onClick={() => { setActionDialogOpen(false); setActionTarget(null); setActionType(null); setActionNote(''); }}>Cancel</Button>
                ...
                }}>{actionType === 'approve' ? 'Approve' : 'Reject'}</Button>
              </div>
            </div>
          ) : (
