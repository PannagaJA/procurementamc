...
export default function DepartmentManagement() {
  ...
  const handleCreate = async () => {
    if (!isAdmin) {
      toast({ title: "Unauthorized", description: "Only admins can create departments", variant: "destructive" });
      return;
    }
    if (!form.location_id || !form.name) {
      toast({ title: "Validation", description: "Select a location and enter a department name", variant: "destructive" });
      return;
    }
    setLoading(true);
    const { data, error } = await supabase.from("departments").insert({ name: form.name.trim(), prefix: form.prefix.trim() || null, location_id: form.location_id }).select().single();
    setLoading(false);
    if (error) {
      console.error(error);
  ...
  const handleUpdate = async (id: string) => {
    if (!isAdmin) {
      toast({ title: "Unauthorized", description: "Only admins can edit departments", variant: "destructive" });
      return;
    }
    if (!editForm.name.trim()) {
      toast({ title: "Validation", description: "Department name cannot be empty", variant: "destructive" });
      return;
    }
    setLoading(true);
  ...
  const handleDelete = async (id: string) => {
    if (!isAdmin) {
      toast({ title: "Unauthorized", description: "Only admins can delete departments", variant: "destructive" });
      return;
    }
    setLoading(true);
      ...
      <div className="container mx-auto py-8">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Add departments under a specific location (Admin only)</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <Input value={form.prefix} onChange={(e) => setForm((p) => ({ ...p, prefix: e.target.value }))} className="h-10" />
              </div>
            </div>
            ...
            <div className="mt-6">
              ...
              <div className="space-y-2">
                {departments.length === 0 && <div className="text-sm text-muted-foreground">No departments found for this location.</div>}
                {departments.map((d) => (
                  <div key={d.id} className="p-2 border rounded flex justify-between items-center">
                    <div className="flex-1">
                      {editingId === d.id ? (
                        <div className="grid grid-cols-2 gap-2">
                          ...
                          <Input value={editForm.prefix} onChange={(e) => setEditForm((p) => ({ ...p, prefix: e.target.value }))} className="h-9" />
                        </div>
                      ) : (
                        <>
                          <div className="font-medium">{d.name}</div>
                          {d.prefix && <div className="text-sm text-muted-foreground">Prefix: {d.prefix}</div>}
                        </>
                      )}
                    </div>
                    <div className="ml-4 flex items-center gap-2">
                      {editingId === d.id ? (
                        ...
                        isAdmin ? (
                          <>
                            <Button size="sm" onClick={() => startEdit(d)}>Edit</Button>
                            <AlertDialog>
