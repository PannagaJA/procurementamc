...
export const uploadImage = async (
  file: File,
  bucket: string = "inventory-images",
): Promise<string | null> => {
...
export const inventoryApi = {
    ...
    try {
      ...
      if (search) {
        dataQuery = dataQuery.or(
          `item_code.ilike.%${search}%,item_name.ilike.%${search}%,department.ilike.%${search}%`,
        );
      }
  ...
  async getInventoryItem(id: string) {
    try {
      ...
      const [{ data: itemData, error: itemError }, { data: historyData, error: historyError }] =
        await Promise.all([itemQuery, historyQuery]);
  ...
  async getCategoriesForDepartment(departmentName?: string) {
    try {
      let query: any = supabase.from("inventory").select("category_id, categories(name)");
