...
export const libraryApi = {
  ...
  async getBooks({
    ...
    try {
      ...
      const { data, error, count } = await query.order("title").range(offset, offset + limit - 1);
  ...
  async issueBook(payload: any) {
    try {
      ...
      const updated = books.map((b: any) =>
        b.id === payload.bookId ? { ...b, available: (b.available || 0) - 1 } : b,
      );
      localStorage.setItem(LS_KEYS.BOOKS, JSON.stringify(updated));
      return rec;
    }
  ...
  async returnBook(issueId: string) {
    try {
      ...
      const books = JSON.parse(localStorage.getItem(LS_KEYS.BOOKS) || "[]");
      const newBooks = books.map((b: any) =>
        b.id === issue.bookId ? { ...b, available: (b.available || 0) + 1 } : b,
      );
