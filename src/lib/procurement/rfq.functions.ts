...
export const createRfq = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    const minQuotes = Math.max(
      3,
      ...(rules ?? []).map((r: any) => Number(r.min_quotations || 3)),
    );
...
export const recordQuotationResponse = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (rvErr || !rfqVendor) {
      return { ok: false as const, error: 'Vendor is not invited to this RFQ', code: 'vendor_not_invited' };
    }

    // Insert or replace quotation lines
    if (data.lines && data.lines.length > 0) {
      // Clean up previous lines for this vendor & rfq if any
      await db.from('quotation_lines').delete().eq('rfq_id', data.rfq_id).eq('vendor_id', data.vendor_id);
