...
export const requestEmergencyProcurement = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (cost <= 0) {
      return { ok: false as const, error: 'Estimated cost must be greater than zero.', code: 'invalid_cost' };
    }
    ...
    if (projectedTotal > cap) {
      ...
      return {
        ok: false as const,
        error: `Emergency procurement request of ₹${cost.toLocaleString('en-IN')} is HARD-BLOCKED. ` +
...
export const approveEmergency = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (epErr || !ep) return { ok: false as const, error: 'Emergency procurement record not found' };
    ...
    if (ep.is_post_facto && data.decision === 'approved') {
      ...
      if (diffHours > 48) {
        console.warn(`Post-facto ratification warning: Approved ${diffHours.toFixed(1)} hours after register entry (exceeds 48h guideline).`);
      }
    }
...
export const listEmergencyProcurements = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data, context }) => {
    ...
    return {
      ok: true as const,
      financialYear: fy,
      ledger: ledgerRes.data || { financial_year: fy, running_total: 0, cap_limit: EMERGENCY_CAP_LIMIT },
