...
export const applyEmpanelment = createServerFn({ method: 'POST' })
  ...
  .inputValidator((input: VendorApplicationInput) => input)
  .handler(async ({ data, context }) => {
    ...
    if (data.documents?.length) {
      await db.from('vendor_documents').insert(
        data.documents.map((d) => ({ vendor_id: vendor.id, doc_type: d.doc_type, file_url: d.file_url })),
      );
    }
...
export const evaluateVendor = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    const d = deny(held, ['procurement_officer', 'procurement_executive', 'purchase_committee'], 'evaluate vendors');
    if (d) return d;
...
export const approveEmpanelment = createServerFn({ method: 'POST' })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: { vendor_id: string; notes?: string | null }) => input)
  .handler(async ({ data, context }) => {
    const { db, held } = await rolesOf(context);
    ...
    if (!existing || existing.length === 0) {
      return { ok: false as const, error: 'This vendor has not been evaluated yet. Evaluation must precede empanelment.' };
    }
...
export const rejectEmpanelment = createServerFn({ method: 'POST' })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: { vendor_id: string; reason: string }) => input)
  .handler(async ({ data, context }) => {
    const { db, held } = await rolesOf(context);
    const d = deny(held, ['evp'], 'reject vendor empanelment');
    if (d) return d;
    const { error } = await db.from('vendors').update({ status: 'rejected' }).eq('id', data.vendor_id);
    if (error) return { ok: false as const, error: error.message };
...
export const suspendVendor = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (d) return d;
    const { error } = await db.from('vendors').update({ status: 'suspended' }).eq('id', data.vendor_id);
    if (error) return { ok: false as const, error: error.message };
    return { ok: true as const };
  });
...
export const blacklistVendor = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (d) return d;
    const { error } = await db.from('vendors').update({ status: 'debarred' }).eq('id', data.vendor_id);
    if (error) return { ok: false as const, error: error.message };
...
export const listVendorsForReview = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ context }) => {
    ...
    const { data } = await db
      .from('vendors')
      .select('id, name, status, gst_number, pan_number, empanelled_on, empanelment_expiry, created_at')
