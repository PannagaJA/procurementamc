...
export async function assertPrApproved(db: Db, prId: string): Promise<void> {
  ...
  if (error)
    throw ruleError(
      "pr_lookup_failed",
      `Could not read the purchase requisition: ${error.message}`,
    );
...
export async function assertNoPoSplitting(
  ...
  if (prErr)
    throw ruleError(
      "pr_lookup_failed",
      `Could not read the purchase requisition: ${prErr.message}`,
    );
  ...
  if (poErr && !/does not exist|schema cache/i.test(poErr.message ?? "")) {
    throw ruleError(
      "po_lookup_failed",
      `Could not read existing purchase orders: ${poErr.message}`,
    );
  }
  ...
  if (vendorId) {
    ...
    if (!rErr && recentVendorPos && recentVendorPos.length > 0) {
      ...
      if (relatedPos.length > 0) {
        const pastSum = relatedPos.reduce(
...
export async function assertThreeWayMatch(
  ...
  const [po, grn, inv] = await Promise.all([
    ...
    db
      ...
      .select("id, po_id, accepted_value, status")
      .eq("id", grnId)
      .maybeSingle(),
  ...
  if (!po?.data)
    throw ruleError("po_not_found", "Three-way match failed: the purchase order does not exist.");
  if (!grn?.data)
    throw ruleError(
      "grn_not_found",
      "Three-way match failed: the goods receipt note does not exist.",
    );
  if (!inv?.data)
    throw ruleError("invoice_not_found", "Three-way match failed: the invoice does not exist.");

  if (grn.data.po_id !== poId) {
    throw ruleError(
      "grn_po_mismatch",
      "Three-way match failed: the goods receipt note belongs to a different purchase order.",
    );
  }
  if (inv.data.po_id !== poId || (inv.data.grn_id && inv.data.grn_id !== grnId)) {
    throw ruleError(
      "invoice_po_mismatch",
      "Three-way match failed: the invoice does not reference this purchase order and receipt note.",
    );
  }
  if (grn.data.status !== "accepted") {
    throw ruleError(
      "grn_not_accepted",
      `Three-way match failed: goods receipt is "${grn.data.status}", not accepted.`,
    );
  }
...
export async function assertEmergencyCapNotExceeded(db: Db, newValue: number): Promise<void> {
  ...
  if (error)
    throw ruleError(
      ...
      `Could not read the emergency procurement register: ${error.message}`,
    );
